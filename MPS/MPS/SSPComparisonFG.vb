...
Public Class SSPComparisonFG
    ...
    Private Sub Report1_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        sqlstr = "select 0 as vendorcode, 'SELECT ALL' as vendorname union all " & _
                 ...
                 " select distinct ssp.vendorcode from ssp as ssp) as foo" & _
    ...
    Private Function GenerateExcel(ByVal list As Integer, ByVal Filename As String, ByVal errmsg As String) As Boolean
        ...
        Try
            ...
            Dim VendorFilter2 As String = String.Empty
            If check.id <> 1 Then
                vendorfilter = " and vendorcode = " & check.id
                VendorFilter2 = " and e.vendorcode = " & check.id
            End If
            'viewname = sopall
            'Dim sqlstr As String = "(SELECT period as ""Period"", sopfamily as ""Sop Family"" ,  cmmf as ""CMMF"",  materialdesc as ""Material Description"", vendorcode as ""Vendor Code"", vendorname as ""Vendor Name"", market as ""Market"", startingdate::date as ""Starting Date"",  week as ""Week"", orderconfirmed as ""OrderConfirmed"", forecast as ""Forecast"", unit as ""Unit"" from sopallnew" &
            '                        " where period = " & firstVar & vendorfilter & ")"
            Dim sqlstr As String = String.Empty
            StringBuilder1.Append(String.Format("(SELECT period::character varying as ""Period"", sopfamily as ""Sop Family"" ,  cmmf as ""CMMF"",  materialdesc as ""Material Description"", vendorcode as ""Vendor Code"", vendorname as ""Vendor Name"", market as ""Market"", startingdate::date as ""Starting Date"",  week as ""Week"", null::integer as shipqty, orderconfirmed as ""OrderConfirmed"",forecast as ""Forecast"", unit as ""Unit"" , case when orderconfirmed = 0 then 'Forecast' else 'PO' end as status from sopallnew" &
                               " where period = {0} {1} )", firstVar, vendorfilter))
            StringBuilder1.Append(" Union all ")

            StringBuilder1.Append(String.Format("(SELECT period::character varying as ""Period"", sopfamily as ""Sop Family"" , cmmf as ""CMMF"",  materialdesc as ""Material Description"", vendorcode as ""Vendor Code"", vendorname as ""Vendor Name"",market as ""Market"", startingdate::date as ""Starting Date"", week as ""Week"", null::integer as shipqty,orderconfirmed * 1 as ""OrderConfirmed"", forecast * 1 as ""Forecast"",unit as ""Unit"" , case when orderconfirmed = 0 then 'Forecast' else 'PO' end as status from sopallnew " &
                                 " where period >= {0} and period <= {1} {2}  order by ""Period"" desc )", LastVar, firstVar - 1, vendorfilter))

            StringBuilder1.Append(String.Format("union all (with w as(select startdate from weektomonth wk where wk.yearweek = '{0}')" &
                " select 'Shipment' as period, ssf.sopfamily,pd.cmmf,cm.materialdesc,e.vendorcode,v.vendorname,c.customername,od.shipdate,(to_char(date_part('Year',od.shipdate),'FM0000') || to_char(date_part('Week',od.shipdate),'FM00'))::integer as week," &
                " od.deliveredqty,null::integer as orderconfirmed,null::integer as forecast,null::character varying as unit ,'Shipment' as status" &
                " from cxsebodtp od " &
                ...
                " left join customer c on c.customercode = sh.soldtoparty" &
                " LEFT JOIN cmmf cm ON cm.cmmf = pd.cmmf" &
                " left join sspcmmfsop scm on scm.cmmf = pd.cmmf" &
                " left join sspsopfamilies ssf on ssf.sspsopfamilyid = scm.sopfamilyid" &
                " left join cmmfsopmps cs on cs.cmmf = pd.cmmf" &
                ...
                " where od.shipdate >= (select startdate from w) and ordertype = 'Shipment' and pg.groupsbuid = 1 {1})", LastVar, VendorFilter2))


            sqlstr = String.Format("select foo.*,cs.mpsfamily from ({0}) foo left join cmmfsopmps cs on cs.cmmf = foo.""CMMF""", StringBuilder1.ToString)
            bgworker.ReportProgress(3, "DB Query...")
            ExcelStuff.FillDataSource(oWb, 3, sqlstr, dbtools1)
            ...
            oSheet.PivotTables("PivotTable1").CalculatedFields.Add("Requirement", "=OrderConfirmed +Forecast+ shipqty", True)

            'add PageField
            'oSheet.PivotTables("PivotTable1").PivotFields("Market").Orientation = Excel.XlPivotFieldOrientation.xlPageField
            oSheet.PivotTables("PivotTable1").PivotFields("status").Orientation = Excel.XlPivotFieldOrientation.xlPageField
            ...
            oSheet.PivotTables("PivotTable1").PivotFields("mpsfamily").orientation = Excel.XlPivotFieldOrientation.xlRowField
            ...
            oSheet.PivotTables("PivotTable1").pivotfields("mpsfamily").subtotals = {False, False, False, False, False, False, False, False, False, False, False, False}
